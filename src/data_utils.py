...
FEATURE_LIST = [
    "y_cat",  # P Pristine/D Defect: pristine structure (initial structure without any damage)/ damaged structure (sandwich structure with circular face layer debonding)
...
def init_data(syn_data_path=None, real_data_path=None):
    ...
    if syn_data_path is not None:
        url = r"https://zenodo.org/records/6758723/files/data_synthetic.zip"
    ...
    if real_data_path is not None:
        url = r"https://zenodo.org/records/6758723/files/real_world.zip"
        ...
        if type(file_names) == list:
            ...
            dfs = [
                _load_processed_data(
                    file_name, fb, X_col, y_col, to_dB, synthetic, calibration
                )
            ...
            df = _load_processed_data(
                file_names, fb, X_col, y_col, to_dB, synthetic, calibration
            )
...
def load_raw_specs(
    ...
    if df is None:
        ...
        if type(file_names) == list:
            # load dataframes into list
            dfs = [
                _load_raw_specs(file_name, X_col, y_col, synthetic, calibration)
                for file_name in file_names
            ]
...
def _load_raw_specs(
    file_name, X_col="real", y_col=["y_radius"], synthetic=True, calibration=None
):
    if synthetic:
        X_df, y_df = load_raw_data(file_name)
    else:
...
def load_preprocessed_data(
    ...
    if cache:
        pickle_name = Path(
            data_path,
            f"filtered_specs__fmin_{fb.f_min}__fmax_{fb.f_max}__lbins_{fb.n_log_bins}.pkl",
        )
...
def load_syn_reg_data(
    ...
    """
    df, fb = load_preprocessed_data(
        data_path,
        target_col=list(set(target_col + ["y_radius"])),
        f_max=f_max,
        n_log_bins=n_log_bins,
        synthetic=synthetic,
        cache=cache,
    ...
    # round target values
    df["y_radius"] = df["y_radius"].astype(float).round(1)

    # drop very small radii
    df_normal = df[df["y_radius"] == 0.0]
    df_anomaly = df[df["y_radius"] > min_radius]

    Xpos = df_normal.drop(columns=target_col)
    if "y_radius" not in target_col:
        Xpos = Xpos.drop(columns="y_radius")

    Xpos = Xpos.values
    ypos = df_normal[target_col].values

    Xneg = df_anomaly.drop(columns=target_col)
    if "y_radius" not in target_col:
        Xneg = Xneg.drop(columns="y_radius")
    ...
    return X[:, 1:], y.squeeze()


def load_reg_data_base(
    data_path, min_radius=2.5, synthetic=True, cache=False, calibration_file=None
):
    file_names = list(data_path.glob("**/*.csv"))
