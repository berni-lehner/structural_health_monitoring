...
def plot_embedding_targets(X_embedded, y, alpha=1.0, palette=None, markers=None):
    """ """
    fig = plt.figure()
...
def plot_classwise_dist(df, x=None, label_col=None, palette=None):
    ...
    for i, (key, group) in enumerate(df.groupby(label_col)):
        ...
        lbl = (
            f"{key:}_mean"
            if type(df[label_col].values[0]) == str
            else f"{key:.2f}_mean"
        )
        mean_spec = group.drop(columns=label_col).mean(axis=0)
        sns.lineplot(x=x, y=mean_spec, color=palette[i], label=lbl)
        ...
        lbl = (
            f"{key:}_std" if type(df[label_col].values[0]) == str else f"{key:.2f}_std"
        )
        plt.fill_between(
            x, lower_bound, upper_bound, color=palette[i], alpha=0.1, label=lbl
        )

    return fig


def plot_classwise_kde(
    df, label_col, labels, palette=None, feature_idx=0, focus=-1, focus_lw=5
):
    """ """
    fig = plt.figure()
    ...
    for i, (key, group) in enumerate(df.groupby(label_col)):
        ...
        sns.kdeplot(data=group, x=x, linewidth=lw, color=palette[i], label=lbl)
...
def plot_metrics(metrics: pd.DataFrame, palette="Blues", **kwargs):
    """ """
    fig = sns.boxplot(
        x="model", y="values", hue="metrics", palette=palette, data=metrics, **kwargs
    )
...
def plot_metrics_bar(
    metrics: pd.DataFrame, palette="Blues", ci="sd", errwidth=3.0, capsize=0.2
):
    """ """
    fig = sns.barplot(
        x="model",
...
def get_metrics_summary(metrics: pd.DataFrame, summaries=["mean", "std"]):
    """ """
    summary = metrics.groupby(["model", "metrics"])["values"]
    if summaries is not None:
        return summary.agg(summaries)
    return summary.describe()


def get_sorted_metrics_summary(metrics: pd.DataFrame, summaries=["mean", "std"]):
    """ """
    summary = metrics.sort_values(["model", "metrics"]).groupby(["model", "metrics"])[
        "values"
    ]
